...
export const StaffManagement: React.FC = () => {
  ...
  const disableMutation = useMutation({
    mutationFn: (staffId: string) => staffApi.disableStaff(staffId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['staff-list'] });
      setToast({
    ...
    onError: (error: any) => {
      const errorMsg = error.response?.data?.message || 'Lỗi khi vô hiệu hóa';
      setToast({ message: errorMsg, type: 'error' });
      setTimeout(() => setToast(null), 3000);
    },
  });
  // Enable staff mutation
  const enableMutation = useMutation({
    mutationFn: (staffId: string) => staffApi.enableStaff(staffId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['staff-list'] });
      setToast({
    ...
    onError: (error: any) => {
      const errorMsg = error.response?.data?.message || 'Lỗi khi kích hoạt';
      setToast({ message: errorMsg, type: 'error' });
      setTimeout(() => setToast(null), 3000);
    },
  });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <StaffList
          ...
          onDisable={(staffId: string) => disableMutation.mutate(staffId)}
          onEnable={(staffId: string) => enableMutation.mutate(staffId)}
